...
Karousel is an intelligent, client-side job dispenser that accesses a parallelized service without overloading its queue. It dishes out jobs onto a defined number of seats on the 'karousel', tosses out completed jobs, and refills seats with new jobs. As a result, the queue on the parallelized service remains lean and clients make responsible use of the service.
...
A Job class requires the following signature:
    ...
    class MyJob < Karousel::ClientJob
      ...
      def self.populate(number_of_seats)
        ... #returns array of the karousel size (number_of_seats would be 20 in our example)
      end
